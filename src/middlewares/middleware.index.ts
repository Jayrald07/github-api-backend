...
export const handleAuthorization = (request: IRequest, response: Response, next: NextFunction) => {
    try {
...
export const handleRequestLogging = (request: IRequest, response: Response, next: NextFunction) => {
    ...
    logger.info(`Started ${method} ${originalUrl} for ${ip} headers: ${JSON.stringify(headers)}`, { logId });
    ...
    response.on('finish',() => {
        ...
        logger.log(level,`Completed ${response.statusCode} in ${responseTime}ms`, { logId });
    })
